...
Duration: 3 hours Total Marks: 100 Mode: Individual, closed book (course notes allowed,
no AI tools, no internet copy paste) Allowed technologies: HTML and plain JavaScript only.
No CSS styling is required or expected. No frameworks or libraries (no React, jQuery, Boot
strap JS, etc). You may use localStorage as your data store since there is no backend in this
exam.
Scenario
You have been hired to build a lightweight School Management System for a school that wants
everything to run in the browser with no server. The system must support two kinds of users:
students and staff (teachers). Each user type has a different signup and login experience, and
once inside, each role only sees the data it is allowed to see.
Build the system according to the requirements below. Where a requirement is ambiguous,
state the assumption you made in a comment in your code.


Part A: Authentication (25 marks)
1. Build a landing page with two entry points: “I am a Student” and “I am a Staff Member.”
2. Student flow:
• If the student is new (nomatchingrecord exists), they should only be presented with
...
3. Staff flow:
• Staffaccountsarepreregisteredbyanadmin(youcanhardcode2to3staffaccounts
...
1. View a list of only the students in the class they are assigned to teach.
